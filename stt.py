...
text_to_speech = "iconic-aloe-403811-80800b7d7bd7.json"
speech_to_text = "iconic-aloe-403811-fef393854188.json"
...
def main():
    ...
    while True:
        ...
        if (transcript != None):
            ...
            if "탑백 플레이리스트" in transcript or "탑백" in transcript or "인기차트" in transcript or "탑 100" in transcript or "노래" in transcript:
                ...
                for mp3 in mp3_files:
                    path = os.path.join(playlist_path, mp3)
            ...
            if "타이머" in transcript:
                match = re.search(r'(\d+)', transcript) # 시간을 정수로 바꾸기 위함.
                ...
                if match:
                    ...
                    audio_response = generate_audio("다시 입력해주세요")
                    sound = AudioSegment.from_file(io.BytesIO(audio_response), format="wav")
                    play(sound)
                    main()
            ...
            if "날씨" in transcript:
                ...
                params ={'serviceKey' : '6gJTZlpAHG/BsVfQa7V9uirC088uzkuVL6BtWZgJKJQmxYLC3ULsEd0Pkj9bdk77MBBIrAtjjYo4uJ7tzOi/ow==', 'pageNo' : '1', 
                        'numOfRows' : '10', 'dataType' : 'JSON', 'base_date' : base_date, 'base_time' : "0800", 'nx' : '55', 'ny' : '127' }
            ...
            print("인식하지 못했습니다.")
            audio_response = generate_audio("다시 말씀해주세요.")
            sound = AudioSegment.from_file(io.BytesIO(audio_response), format="wav")
            play(sound)
            main()
